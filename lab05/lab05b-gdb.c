...
void pack(InstData *data) {
    ...
    if (data->type == R) {
        ...
        data->rs1 = aplicar_mask(data->rs1, 6);
        pack = somar_bin(pack, data->rs1, 15);
        data->imm = aplicar_mask(data->imm, 13);
        pack = somar_bin(pack, data->imm, 20);
    }
    else if(data->type == S) {
        int imm_temp;
        data->opcode = aplicar_mask(data->opcode, 8);
        pack = somar_bin(pack, data->opcode, 0);
        imm_temp = aplicar_mask(data->imm, 6);
        ...
        data->rs2 = aplicar_mask(data->rs2, 6);
        pack = somar_bin(pack, data->rs2, 20);
        data->imm = aplicar_mask(data->imm >> 5, 8);
        ...
        data->rd = aplicar_mask(data->rd, 6);
        pack = somar_bin(pack, data->rd, 7);
        data->imm = aplicar_mask(data->imm, 21);
        pack = somar_bin(pack, data->imm, 12);
    }
    else if(data->type == J) {
        int imm_temp;
        data->opcode = aplicar_mask(data->opcode, 8);
